...
--delete  AX_Segment_Metric_Facts

insert INTO [AX_Segment_Metric_Facts] 
-------------------------------------------------STAGE 4-----------------------------------------------
...
left join (select w.* /*, CASE WHEN w.[WORKTYPE] in('02') and w.jpnum is not null then 'PM'
	...
	else 'OTH' end 'DEV_worktype'*/ from [dbo].[Client_workorder_PRD] w) 
w on aa.assetnum=w.assetnum where aa.isflag in(1,2)
...
select * into [dbo].[Segment_Objects]
from [LILLY_Repository_New].[dbo].[Segment_Objects_ALL]
where 1=2

--drop table [dbo].[AX_Asset_Saving]
--truncate table [Segment_Objects1]
select * from [Segment_Objects] where SegmentMasterID is not null
--truncate table [Segment_Objects]
...
where aa.isflag in(1,2) --and s.siteid='prd' -----[Effective_Currentflag]='Y'
...
select count(*) from [Segment_Objects]--41114

select count(*) from [Segment_Objects_ALL]--85362
select * from [dbo].[Asset_Master] where siteid not in ('PRD')
