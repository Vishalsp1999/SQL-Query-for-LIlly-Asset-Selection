...
-----------------Saving Query------------------
 select * into [Lilly_Straging].dbo.Saving_2904_1 from(
select j.*,
...
from 
(select SegmentMasterID,SegmentMasterDescription,siteid,isnull([pmlaborcost]/2,0)[pmlaborcost],isnull([pmsparecost]/2,0)[pmsparecost],isnull([pmtotal]/2,0)[pmtotal],isnull([NONpmlaborcost]/2,0)[NONpmlaborcost],isnull([NONpmsparecost]/2,0)[NONpmsparecost],
isnull([NONpmTotal]/2,0)[NONpmTotal],isnull([pmdowntime]/2,0)[pmdowntime],isnull([GBDowntime]/2,0) [GBDowntime]
 from [dbo].[ASMF_ALL]
...
set ax.saving_index=c.Savings_index
 from [dbo].[ASMF_ALL] ax 
join [dbo].[AX_Asset_Saving_all]  c
