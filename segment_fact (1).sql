...
select * into [AX_Segment_Metric_Facts1]
from [Honda_Repository].[dbo].[AX_Segment_Metric_Facts]
where 1=2

select * from [AX_Segment_Metric_Facts1]


insert INTO [AX_Segment_Metric_Facts1] 
-------------------------------------------------STAGE 4-----------------------------------------------
...
sum(w.NONPMDowntime) as NonpmDowntime,
...
update [dbo].[AX_Segment_Metric_Facts1]
set SegmentGroupID=108


select * from [Sherwin_Williams_Repository].[dbo].[Client_Workorder]
 ...
 from [AX_Segment_Metric_Facts1] a
 inner join 
 (
 select row_number() OVER(order by SegmentMasterDescription) [SegmentMasterID] , SegmentMasterDescription 
 from [AX_Segment_Metric_Facts1] )b
 on a.SegmentMasterDescription=b.SegmentMasterDescription

 -------------------------------- Columns required for Asset_Saving  -----------

 select Asset_Master_Group_Details_Desc,pmlaborcost,pmsparecost,pmtotal,NONpmlaborcost,
		NONpmsparecost,NONpmTotal,pmdowntime,GBDowntime
from [dbo].AX_Segment_Metric_Facts
...
select distinct [Jobplanid]  from AX_Client_JOBPLAN where [Jobplanid]  not in (select distinct [job plan] from [Client_PM]) 
select distinct [Jobplanid]  from AX_Client_JOBTASK  where [Jobplanid] not in (select distinct [job plan] from [Client_PM]) 
select distinct [Jobplanid] from AX_Client_JPLongdescription  where [Jobplanid] not in (select distinct [job plan] from [Client_PM]) 


select distinct a.[Jobplanid]  from AX_Client_JOBPLAN a --where [Jobplanid] not in (select distinct [job plan] from [Client_PM]) 
left outer join AX_Client_JOBTASK b
on a.[Jobplanid]=b.[Jobplanid]
where b.[Jobplanid] is null
not in (select distinct [job plan] from [Client_PM]) 
select distinct [Jobplanid] from AX_Client_JPLongdescription  where [Jobplanid] not in (select distinct [job plan] from [Client_PM]) 
